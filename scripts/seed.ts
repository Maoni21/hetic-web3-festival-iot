...
import { Venue } from '../src/domain/entities/Venue.js';
import { TicketId } from '../src/domain/value-objects/TicketId.js';
import { VenueId } from '../src/domain/value-objects/VenueId.js';
import { TicketStatus } from '../src/domain/enums/TicketStatus.js';
import { Timestamp } from '../src/domain/value-objects/Timestamp.js';
import { InMemoryTicketRepository } from '../src/infrastructure/repositories/InMemoryTicketRepository.js';
import { InMemoryVenueRepository } from '../src/infrastructure/repositories/InMemoryVenueRepository.js';
...
export function seedTickets(repository: InMemoryTicketRepository): void {
  ...
  console.log(JSON.stringify({ level: 'INFO', context: 'seed', message: `Seeded ${tickets.length} tickets` }));
}
...
export function seedVenues(repository: InMemoryVenueRepository): void {
  ...
  console.log(JSON.stringify({ level: 'INFO', context: 'seed', message: `Seeded ${venues.length} venues` }));
}
